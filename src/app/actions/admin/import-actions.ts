    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        const productData = {
          ...
          stock_quantity: typeof row.stock_quantity === 'number' ? row.stock_quantity : (row.stock_quantity ? parseInt(row.stock_quantity) : 0),
...
async function parseImportFile(file: File): Promise<ProductImportData[]> {
  ...
  if (file.name.endsWith('.xlsx') || file.name.endsWith('.xls')) {
    ...
    const data: ProductImportData[] = XLSX.utils.sheet_to_json(worksheet, { raw: false })
    return data
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Convert types
      if (row.price) {
        row.price = typeof row.price === 'number' ? row.price : parseFloat(row.price)
      }
      
      if (row.stock_quantity) {
        row.stock_quantity = typeof row.stock_quantity === 'number' ? row.stock_quantity : 
