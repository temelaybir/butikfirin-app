...
export class AdminAuthService {
  ...
  async login(credentials: LoginCredentials, request?: Request): Promise<LoginResult> {
    try {
      ...
      let sessionData;
      try {
        sessionData = await this.createSession(user.id, request)
      } catch (sessionError) {
        console.error('Session creation failed:', sessionError)
        // Fallback to simple auth if session table is missing
        return { 
          success: false, 
          error: 'Admin oturumu oluşturulamadı. Lütfen veritabanı tablolarını kontrol edin.' 
        }
      }
