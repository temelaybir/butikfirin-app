...
import { ArrowUpRight, CreditCard, Package, ShoppingCart, Truck } from 'lucide-react'
import Link from 'next/link'
import { useEffect, useState } from 'react'

interface EnhancedOrderIconProps {
  onClick?: () => void
...
export function EnhancedOrderIcon({
  onClick,
  variant = 'default',
  showDropdown = false,
  className
}: EnhancedOrderIconProps) {
  const { cart } = useCart()
  ...
  // Sipariş değiştiğinde animasyon tetikle
  useEffect(() => {
  ...
  if (variant === 'minimal') {
    return (
      <Button
        ...
        aria-label={`Sipariş - ${cart.totalItems || 0} ürün`}
      >
            ...
            <CardContent className="p-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold">Siparişiniz</h3>
  ...
  return (
    ...
    >
      ...
      <div className="hidden sm:flex flex-col items-start">
        <span className="text-xs text-muted-foreground">Siparişim</span>
...
// Enhanced Order Button with Price Display
export function OrderButton() {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start">
        <span className="text-sm font-medium">
          Sipariş {cart.totalItems > 0 && `(${cart.totalItems})`}
        </span>
...
export const EnhancedCartIcon = EnhancedOrderIcon
export const CartButton = OrderButton