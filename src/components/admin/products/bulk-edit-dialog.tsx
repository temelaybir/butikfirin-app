...
export function BulkEditDialog({
  ...
  const { execute: executeBulkUpdate, loading: isBulkUpdateLoading } = useActionHandler({
    ...
    try {
      switch (operation) {
        ...
        case 'delete':
          ...
          await executeBulkUpdate(bulkDeleteProducts(selectedProductIds.map(id => Number(id))))
          break
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px]">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={handleClose} disabled={isBulkUpdateLoading}>
            İptal
          </Button>
          <Button 
            onClick={handleSave} 
          ...
          >
            {isBulkUpdateLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
