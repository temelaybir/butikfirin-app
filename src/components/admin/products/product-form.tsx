...
export function ProductForm({ product, onSuccess, onCancel, onError }: ProductFormProps) {
  ...
  const generateSlug = async (name: string) => {
    if (!name) return
    try {
      const slug = await generateSlugAction(name, product?.id ? parseInt(product.id) : undefined)
  ...
  const checkSKU = async (sku: string) => {
    if (!sku) return
    try {
      const result = await checkSKUAction(sku, product?.id ? parseInt(product.id) : undefined)
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (product) {
        const result = await update.execute(updateProduct(parseInt(product.id), formData))
        if (result.success && result.data) {
          onSuccess?.(result.data)
        } else if (result.success) {
          // Handle case where data is undefined (shouldn't happen, but just in case)
